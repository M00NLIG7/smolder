...
use smolder_core::prelude::{
    Connection, DurableOpenOptions, NtlmAuthenticator, ResilientHandle, TokioTcpTransport,
    TreeConnected,
};
...
async fn reopens_durable_handle_after_transport_reconnect_when_configured() {
    ...
    {
        eprintln!(
            "skipping Windows durable reconnect test: server did not advertise leasing support"
        );
        return;
    }

    let path = unique_path_in_dir("smolder-win-reconnect", &config.test_dir);
