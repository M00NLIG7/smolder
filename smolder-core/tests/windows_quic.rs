...
impl WindowsQuicConfig {
    fn from_env() -> Option<Self> {
        let server = required_env("SMOLDER_WINDOWS_QUIC_SERVER")?;
        Some(Self {
            connect_host: required_env("SMOLDER_WINDOWS_QUIC_CONNECT_HOST")
                .unwrap_or_else(|| server.clone()),
            tls_server_name: required_env("SMOLDER_WINDOWS_QUIC_TLS_SERVER_NAME")
                .unwrap_or_else(|| server.clone()),
            port: required_env("SMOLDER_WINDOWS_QUIC_PORT")
                .and_then(|value| value.parse::<u16>().ok())
                .unwrap_or(443),
            username: required_env("SMOLDER_WINDOWS_QUIC_USERNAME")?,
            password: required_env("SMOLDER_WINDOWS_QUIC_PASSWORD")?,
            share: required_env("SMOLDER_WINDOWS_QUIC_SHARE")?,
            test_dir: required_env("SMOLDER_WINDOWS_QUIC_TEST_DIR").unwrap_or_default(),
            domain: required_env("SMOLDER_WINDOWS_QUIC_DOMAIN"),
            workstation: required_env("SMOLDER_WINDOWS_QUIC_WORKSTATION"),
            server,
        })
    }
...
fn windows_quic_lock() -> &'static Mutex<()> {
...
fn unique_test_file_path(test_dir: &str) -> String {
    let stamp = SystemTime::now()
        ...
        .as_nanos();
    let file_name = format!("smolder-win-quic-{}-{stamp}.txt", std::process::id());
...
async fn authenticates_and_connects_tree_over_quic_when_configured() {
    let _guard = windows_quic_lock().lock().await;
    let Some(config) = WindowsQuicConfig::from_env() else {
        eprintln!(
